...
async def show_collection_with_ids(user_id: int) -> str:
    ...
    rarity_order = ["Легендарная", "Блещет умом", "Эпическая", "Редкая", "Обычная"]
...
async def get_card(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if current_time - last_drop < 6 * 3600:  # 6 часов
        next_time = datetime.fromtimestamp(last_drop + 6 * 3600)
        await update.message.reply_text(
            f"⏳ Следующую карточку можно получить: {next_time.strftime('%H:%M %d.%m.%Y')}"
        )
        return
...
async def admin_card_name(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text("Введите редкость карточки (Легендарная/Блещет умом/Эпическая/Редкая/Обычная):")
...
async def admin_card_rarity(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    valid_rarities = ["Легендарная", "Блещет умом", "Эпическая", "Редкая", "Обычная"]
    
    if rarity not in valid_rarities:
        await update.message.reply_text("❌ Некорректная редкость! Используйте: Легендарная/Блещет умом/Эпическая/Редкая/Обычная")
        return ADMIN_CARD_RARITY
...
def main() -> None:
    ...
    addcard_conv_handler = ConversationHandler(
        ...
        states={
            ...
            ADMIN_CARD_IMAGE: [MessageHandler(filters.PHOTO, admin_card_image)],
        },
