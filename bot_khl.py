...
def _run_bot_main():
    ...
    while True:
        try:
            ...
            # /update завершает процесс через os._exit(0), а явный sys.exit тоже не нужно
            # превращать в бесконечный цикл. Ненулевой код пишем в crash-log.
            if getattr(e, "code", 0):
                _err = traceback.format_exc()
                print(_err)
                try:
                    with open(os.path.join(BASE_DIR, "bot_crash.log"), "a", encoding="utf-8") as f:
                        f.write(f"\n\n===== {datetime.now().isoformat()} SystemExit =====\n")
                        f.write(_err)
                except Exception:
                    pass
            raise
            ...
            try:
                ...
                pass
            # Частая причина "сам выключился" у long polling — временный сетевой сбой,
            # конфликт getUpdates после рестарта или ошибка Telegram API. Через 10 секунд
            # поднимаем polling заново, вместо окончательного выхода процесса.
            time.sleep(10)
